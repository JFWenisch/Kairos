# Kairos Documentation

Welcome to the official documentation for Kairos.

This site is built automatically from the Markdown files in this folder and published to GitHub Pages.

## Start Here

- [Quick Start](quickstart.md)
- [Troubleshooting](troubleshooting.md)

## Guides

- [Configuration](configuration.md)
- [Authentication](authentication.md)
- [REST API](api.md)
- [Import / Export](importexport.md)
- [Docker Pullability](docker-pullability.md)
- [Announcements](announcements.md)
- [Embed Status Widget](embed.md)

## Project Links

- Repository: https://github.com/wenisch-tech/Kairos
- Documentations: https://kairos.wenisch.tech/docs
- Container image: https://github.com/wenisch-tech/Kairos/pkgs/container/kairos
- Helm chart: https://artifacthub.io/packages/helm/wenisch-tech/kairos
